...
def main():
    ...
    # «bot» и «links» телефона не касаются вовсе: первый только принимает
    # сообщения и складывает ссылки, второй читает базу. Требовать кабель
    # для них — значит не дать включить приёмник там, где телефона нет
    # (а это ровно то, ради чего отдельная команда и заведена).
